...
class Manufacturer(TimeStampedModel):
    name = models.CharField(
        verbose_name="Manufacturer Name", max_length=100, unique=True
    )
...
class DataSource(TimeStampedModel):
    ...
    )  # how to show that the datasource was entered by a user?
...
class Drug(TimeStampedModel):
    ...
    manufacturer_name = models.ForeignKey(
        to=Manufacturer, on_delete=models.PROTECT, related_name="manufacturer"
    )
    drug_type = models.ForeignKey(
        to=DrugType, on_delete=models.PROTECT, related_name="drug_type"
    )
    pack_size_label = models.ForeignKey(
        to=PackSizeLabel, on_delete=models.PROTECT, related_name="packsize"
    )
    price = models.DecimalField(max_digits=7, decimal_places=2, null=True)
    rx_required = models.BooleanField(verbose_name="Prescription Required", null=True)
    short_composition = models.ForeignKey(
        to=DrugComposition, on_delete=models.PROTECT, related_name="composition"
    )
    is_discontinued = models.BooleanField()
    data_source = models.ForeignKey(
        to=DataSource, on_delete=models.PROTECT, related_name="source"
    )
